...
import { addPlanner, getAllSubjects, getAllBoards, getAllStandards } from "../../../services/adminService";
import Toast from "../../Common/Toast";

const AddPlannerModal = ({ onClose, onSuccess }) => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "planner_pdf") {
      setFormData((prev) => ({ ...prev, planner_pdf: files[0] || null }));
    } else {
  ...
  const validate = () => {
    ...
    if (!formData.planner_pdf) newErrors.planner_pdf = "PDF file is required";
    setErrors(newErrors);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      fd.append("planner_pdf", formData.planner_pdf);

      await addPlanner(fd);
      setToast({ show: true, message: "Planner added successfully", type: "success" });
      setTimeout(() => onSuccess(), 900);
    } catch (error) {
      setToast({
        show: true,
        message: error.response?.data?.error || error.response?.data?.message || "Failed to add planner",
        type: "error",
      });
    } finally {
      setLoading(false);
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
        <div className="bg-white rounded-2xl p-6 max-w-md w-full my-8">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-2xl font-bold text-gray-800">Add Planner</h3>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">
                Planner PDF <span className="text-red-500">*</span>
              </label>
              <input
                type="file"
                name="planner_pdf"
                ...
                className={`w-full px-4 py-3 border-2 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition outline-none ${
                  errors.planner_pdf ? "border-red-300" : "border-gray-200"
                }`}
              />
              {errors.planner_pdf && <p className="mt-1 text-sm text-red-600">{errors.planner_pdf}</p>}
            </div>
