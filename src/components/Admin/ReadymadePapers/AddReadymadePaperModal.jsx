...
const AddReadymadePaperModal = ({ onClose, onSuccess }) => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "paper_pdf" || name === "paper_word") {
  ...
  const validate = () => {
    ...
    if (!formData.paper_pdf && !formData.paper_word) {
      newErrors.paper_pdf = "Upload at least one file (PDF or Word)";
    }
    setErrors(newErrors);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.paper_pdf) {
        formDataToSend.append("paper_pdf", formData.paper_pdf);
      }
      if (formData.paper_word) {
        formDataToSend.append("paper_word", formData.paper_word);
      }

      await addReadymadePaper(formDataToSend);
      setToast({ show: true, message: "Readymade paper added successfully", type: "success" });
      setTimeout(() => {
        onSuccess();
      }, 1000);
    } catch (error) {
      setToast({
        show: true,
        message: error.response?.data?.error || error.response?.data?.message || "Failed to add readymade paper",
        type: "error",
      });
    } finally {
      setLoading(false);
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
        <div className="bg-white rounded-2xl p-6 max-w-md w-full my-8">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-2xl font-bold text-gray-800">Add Readymade Paper</h3>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-3 border-2 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition outline-none ${
                  errors.paper_pdf ? "border-red-300" : "border-gray-200"
                }`}
              />
            </div>
            ...
            <div>
              ...
              />
              {errors.paper_pdf && <p className="mt-1 text-sm text-red-600">{errors.paper_pdf}</p>}
              <p className="mt-1 text-xs text-gray-500">
                Upload a PDF and/or a Word file — at least one is required.
              </p>
            </div>
...
export default AddReadymadePaperModal;
