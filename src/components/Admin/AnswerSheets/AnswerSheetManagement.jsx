...
const AnswerSheetManagement = () => {
  ...
  const toggleSelectAll = () => {
    const visibleIds = filteredSheets.map((s) => s.answer_sheet_id);
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      const res = await bulkDeleteAnswerSheets(selectedIds);
      setToast({
        show: true,
        message: `Deleted ${res?.deletedCount ?? selectedIds.length} answer sheet(s) successfully`,
        type: "success",
      });
      ...
      setToast({
        show: true,
        message: error.response?.data?.message || "Failed to delete answer sheets",
        type: "error",
      });
    } finally {
  ...
  return (
    <>
      ...
      {filteredSheets.length > 0 && (
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3 px-4 py-3 bg-white border border-gray-200 rounded-xl shadow-sm">
          ...
          >
            ...
            <span>
              {filteredSheets.every((s) => selectedIds.includes(s.answer_sheet_id))
                ? "Deselect all"
                : "Select all"}
            </span>
          </button>
          ...
          filteredSheets.map((sheet) => (
            <div
              ...
              className={`relative bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition ${
                selectedIds.includes(sheet.answer_sheet_id)
            ...
            >
              ...
              <label className="absolute top-3 left-3 z-10 bg-white/90 rounded-md p-1 shadow cursor-pointer">
                <input
                  ...
                  checked={selectedIds.includes(sheet.answer_sheet_id)}
                  onChange={() => toggleSelect(sheet.answer_sheet_id)}
                />
              </label>
      ...
      {showBulkDeleteModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full mx-4">
            <h3 className="text-xl font-bold text-gray-800 mb-4">Delete Selected Answer Sheets</h3>
            <p className="text-gray-600 mb-6">
              Are you sure you want to delete {selectedIds.length} selected answer sheet(s)? This action cannot be undone.
            </p>
            <div className="flex gap-4">
              <button
