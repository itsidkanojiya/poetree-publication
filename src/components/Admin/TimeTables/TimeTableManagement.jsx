...
const TimeTableManagement = () => {
  ...
  const fetchItems = async () => {
    try {
      ...
      setToast({ show: true, message: "Failed to load time tables", type: "error" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    if (!selected) return;
    try {
      await deleteTimeTable(selected.timetable_id);
      setToast({ show: true, message: "Time table deleted successfully", type: "success" });
      ...
      setToast({
        show: true,
        message: error.response?.data?.message || "Failed to delete time table",
        type: "error",
      });
    }
  };
  ...
  const toggleSelectAll = () => {
    const ids = filtered.map((p) => p.timetable_id);
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      const res = await bulkDeleteTimeTables(selectedIds);
      setToast({
        show: true,
        message: `Deleted ${res?.deletedCount ?? selectedIds.length} time table(s) successfully`,
        type: "success",
      });
      ...
      setToast({
        show: true,
        message: error.response?.data?.message || "Failed to delete time tables",
        type: "error",
      });
    } finally {
    ...
    <>
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Upload time table PDFs for one or more standards</p>
        </div>
        <button
      ...
      {filtered.length > 0 && (
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3 px-4 py-3 bg-white border border-gray-200 rounded-xl shadow-sm">
          ...
          >
            ...
            <span>{filtered.every((p) => selectedIds.includes(p.timetable_id)) ? "Deselect all" : "Select all"}</span>
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.length === 0 ? (
          <div className="col-span-full bg-white rounded-2xl shadow-lg p-12 text-center">
            ...
            <p className="text-gray-500 text-lg">No time tables found</p>
          </div>
        ) : (
          filtered.map((p) => (
            <div
              ...
              className={`relative bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition ${
                selectedIds.includes(p.timetable_id) ? "ring-2 ring-blue-500" : ""
              }`}
            >
              <label className="absolute top-3 left-3 z-10 bg-white/90 rounded-md p-1 shadow cursor-pointer">
                <input
                  ...
                  checked={selectedIds.includes(p.timetable_id)}
                  onChange={() => toggleSelect(p.timetable_id)}
                />
              </label>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full mx-4">
            <h3 className="text-xl font-bold text-gray-800 mb-4">Delete Time Table</h3>
            <p className="text-gray-600 mb-6">Are you sure you want to delete this time table? This cannot be undone.</p>
      ...
      {showBulkDeleteModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full mx-4">
            <h3 className="text-xl font-bold text-gray-800 mb-4">Delete Selected Time Tables</h3>
            <p className="text-gray-600 mb-6">
              Are you sure you want to delete {selectedIds.length} selected time table(s)? This cannot be undone.
            </p>
            <div className="flex gap-4">
              <button
