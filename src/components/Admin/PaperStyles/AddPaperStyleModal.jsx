...
import { addPaperStyle, getAllSubjects, getAllBoards, getAllStandards } from "../../../services/adminService";
import Toast from "../../Common/Toast";

const AddPaperStyleModal = ({ onClose, onSuccess }) => {
  const [formData, setFormData] = useState({
    title: "",
    subject_id: "",
    board_id: "",
    standard: [],
    paper_style_pdf: null,
  });
  ...
  const handleChange = (e) => {
    ...
    if (name === "paper_style_pdf") {
      setFormData((prev) => ({ ...prev, paper_style_pdf: files[0] || null }));
    } else {
  ...
  const validate = () => {
    ...
    if (!formData.paper_style_pdf) newErrors.paper_style_pdf = "PDF file is required";
    setErrors(newErrors);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      fd.append("paper_style_pdf", formData.paper_style_pdf);

      await addPaperStyle(fd);
      setToast({ show: true, message: "Paper style added successfully", type: "success" });
      setTimeout(() => onSuccess(), 900);
    } catch (error) {
      setToast({
        show: true,
        message: error.response?.data?.error || error.response?.data?.message || "Failed to add paper style",
        type: "error",
      });
    } finally {
      setLoading(false);
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
        <div className="bg-white rounded-2xl p-6 max-w-md w-full my-8">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-2xl font-bold text-gray-800">Add Paper Style</h3>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">
                Paper Style PDF <span className="text-red-500">*</span>
              </label>
              <input
                type="file"
                name="paper_style_pdf"
                onChange={handleChange}
                accept=".pdf"
                className={`w-full px-4 py-3 border-2 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition outline-none ${
                  errors.paper_style_pdf ? "border-red-300" : "border-gray-200"
                }`}
              />
              {errors.paper_style_pdf && <p className="mt-1 text-sm text-red-600">{errors.paper_style_pdf}</p>}
            </div>
            ...
            <div className="flex gap-4 pt-4">
              ...
              >
                {loading ? "Uploading..." : "Add Paper Style"}
              </button>
            </div>
          </form>
        </div>
...
export default AddPaperStyleModal;
