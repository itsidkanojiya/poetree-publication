...
import { getQuizWithQuestions, suggestMcq, updateQuiz } from "../services/quizService";
import {
  ArrowLeft,
  Check,
  FileText,
...
const EditQuiz = () => {
  ...
  useEffect(() => {
    let cancelled = false;
    apiClient.get("/auth/my-selections/approved").then((res) => {
      ...
      (data.subject_titles || []).forEach((item) => {
        ...
        if (titleId && titleName) titles.push({ id: titleId, name: titleName, subject_id: subId, subject_name: subName });
      });
      if (!cancelled) setApprovedSubjectTitles(titles);
    }).catch(() => { if (!cancelled) setApprovedSubjectTitles([]); });
    return () => { cancelled = true; };
  }, []);

  useEffect(() => {
    Promise.all([apiClient.get("/boards"), getAllStandards()])
      .then(([boardsRes, standardsRes]) => {
  ...
  useEffect(() => {
    ...
    getQuizWithQuestions(id)
      .then((data) => {
        ...
        setPaperTitle(paper?.paper_title ?? paper?.title ?? "");
        setDate(paper?.date ? paper.date.split("T")[0] : new Date().toISOString().split("T")[0]);
        setTiming(paper?.timing ?? "");
        setSubjectTitleId(paper?.subject_title_id ?? "");
        setChapterId(paper?.chapter_id ?? "");
        setStandard(String(paper?.standard ?? ""));
        setBoardId(paper?.board_id ?? paper?.board ?? "");
        const qs = data?.questions ?? data?.question ?? paper?.questions ?? [];
        const list = Array.isArray(qs) ? qs : [];
        setSuggestedQuestions(list);
        const ids = new Set(list.map((q) => q.question_id ?? q.id).filter(Boolean));
        setSelectedIds(ids);
      })
      .catch(() => {
        if (!cancelled) setToast({ type: "error", message: "Failed to load quiz" });
      })
      .finally(() => {
  ...
  const handleSuggest = async () => {
    ...
    try {
      const params = { count: Math.min(100, Math.max(1, Number(count) || 10)) };
  ...
  const selectAll = () => setSelectedIds(new Set(suggestedQuestions.map((q) => q.question_id ?? q.id).filter(Boolean)));
  ...
  const handleSave = async () => {
    ...
    try {
      const metadata = {
        ...
        subject: selectedTitle?.subject_name || "",
        board: boardId ? boards.find((b) => String(b.board_id ?? b.id) === String(boardId))?.board_name : undefined,
        subject_title_id: subjectTitleId || undefined,
        chapter_id: chapterId || undefined,
      };
      await updateQuiz(id, metadata, ids);
      setToast({ type: "success", message: "Quiz updated" });
      navigate(`/dashboard/quizzes/${id}`);
    } catch (err) {
      setToast({
        type: "error",
        message: err.response?.data?.message || err.message || "Failed to update quiz",
      });
    } finally {
  ...
  return (
    <div className="min-h-screen p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <p className="text-gray-600 mb-8">Change filters, add/remove questions, update metadata.</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => { setSubjectTitleId(e.target.value); setChapterId(""); }}
                className="w-full border border-gray-300 rounded-lg px-3 py-2"
              >
                <option value="">Select subject title</option>
                {approvedSubjectTitles.map((t) => (
                  <option key={t.id} value={t.id}>{t.name} {t.subject_name ? `(${t.subject_name})` : ""}</option>
                ))}
              </select>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Chapter (optional)</label>
              <select value={chapterId} onChange={(e) => setChapterId(e.target.value)} className="w-full border border-gray-300 rounded-lg px-3 py-2">
            ...
            <div>
              ...
              <select value={standard} onChange={(e) => setStandard(e.target.value)} className="w-full border border-gray-300 rounded-lg px-3 py-2">
            ...
            <div>
              ...
              <select value={boardId} onChange={(e) => setBoardId(e.target.value)} className="w-full border border-gray-300 rounded-lg px-3 py-2">
                <option value="">Any</option>
                {boards.map((b) => (
                  <option key={b.board_id ?? b.id} value={b.board_id ?? b.id}>{b.board_name ?? b.name}</option>
                ))}
              </select>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Number of MCQs to suggest</label>
              <input type="number" min={1} max={100} value={count} onChange={(e) => setCount(Number(e.target.value) || 10)} className="w-full border border-gray-300 rounded-lg px-3 py-2" />
            </div>
          </div>
          <button onClick={handleSuggest} disabled={suggestLoading} className="inline-flex items-center gap-2 px-4 py-2 bg-amber-500 text-white rounded-lg hover:bg-amber-600 font-medium disabled:opacity-50">
            {suggestLoading ? <Loader className="w-5 h-5 animate-spin" /> : <Sparkles className="w-5 h-5" />}
            {suggestLoading ? "Loading..." : "Suggest more MCQs"}
          </button>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              <button onClick={selectAll} className="text-sm text-amber-600 hover:underline">Select all</button>
                ...
                >
                  <div className="flex gap-3">
                    <div className="flex-shrink-0 mt-0.5">
                      <div className={`w-5 h-5 rounded border-2 flex items-center justify-center ${checked ? "bg-amber-500 border-amber-500" : "border-gray-400"}`}>
                      ...
                      {opts.length > 0 && (
                        <ul className="mt-2 text-sm text-gray-600 space-y-1">
                          {opts.slice(0, 10).map((o, i) => (
                            <li key={i}>{String.fromCharCode(65 + i)}. {typeof o === "object" ? (o.option ?? o.text ?? "") : o}</li>
                          ))}
                        </ul>
                      )}
                    </div>
          ...
          <div className="mt-4">
            ...
            <input type="text" value={paperTitle} onChange={(e) => setPaperTitle(e.target.value)} placeholder="Quiz title" className="w-full border border-gray-300 rounded-lg px-3 py-2" />
          </div>
          <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="text" value={timing} onChange={(e) => setTiming(e.target.value)} placeholder="e.g. 45 min" className="w-full border border-gray-300 rounded-lg px-3 py-2" />
            </div>
          </div>
          <div className="mt-6">
            <button onClick={handleSave} disabled={saveLoading || selectedIds.size === 0} className="inline-flex items-center gap-2 px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 font-medium disabled:opacity-50">
              {saveLoading ? <Loader className="w-5 h-5 animate-spin" /> : <FileText className="w-5 h-5" />}
              {saveLoading ? "Saving..." : "Update Quiz"}
            </button>
          </div>
        </div>
      </div>
