...
- **Header** (school logo + school name): on the **first page only**.
...
- Behavior:
  ...
  - Load **branding** for this user: school name and school logo URL or file path. If the app has a `schools` table and `user.school_id`, use school-level branding; otherwise use user profile fields (e.g. `school_name`, logo from user).
  - Call the **PDF personalization module** (see below) with: canonical PDF stream/path, branding payload (school name, logo path/URL).
...
- Create a **new module/service** (e.g. `services/worksheetPersonalization.js` or `lib/pdfPersonalization.js`) that:
  - **Input:** (1) Canonical PDF (stream or file path), (2) Branding object: `{ schoolName: string, logoPathOrUrl?: string }` (and optionally tagline, primary color later).
  - **Behavior — two distinct overlays:**
    1. **Header (first page only):** Open the canonical PDF (do not modify the original file). On **page 1 only**, add a **header band** (e.g. top 20–30 mm) containing: school logo on the left (if provided), resized to max height (e.g. 15–20 mm); school name as text (center or next to logo). Sanitize school name. If no logo, render only school name (or placeholder "School Name"). **Do not add this header on page 2, 3, 4, etc.**
...
- Create or reuse a **helper** that, given `userId` (and optionally `schoolId`), returns:
  - `{ schoolName: string, logoPathOrUrl: string | null }`
- If using `schools` table: load school by `user.school_id` and use `school.name`, `school.logo_url` (or resolved path). If `school_id` is null, fallback to `user.school_name` and user-level logo if stored.
...
- [ ] PDF personalization module: **header** (logo + school name) on **first page only**; **watermark** (faint school name/logo) on **every page**; no modification of original file.
